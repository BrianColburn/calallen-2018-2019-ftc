 ...
 * Modified by Brian on 10/25/2018.
 */

public class GoldAlignDetector extends DogeCVDetector implements DistanceSensor {
    ...
    private int     aligned  = 0; // Is the gold mineral aligned
    ...
    public Mat process(Mat input) {
        ...
        for(MatOfPoint cont : contoursYellow){
            ...
            if(score < bestDiffrence && 2550/Math.min(rect.width, rect.height) < 70){
                bestDiffrence = score;
                bestRect = rect;
            }
        }
        ...
        if(bestRect != null){
            ...
            goldXPos = xPos;
            goldYPos = 480 - (bestRect.y + bestRect.height / 2);
    ...
    public void useDefaults() {
        ...
        /*addScorer(new DogeCVScorer() {
            @Override
            public double calculateScore(Mat input) {
                Rect r = Imgproc.boundingRect((MatOfPoint) input);
                return Math.max(0,70-Math.max(r.width, r.height));
            }
        });*/
    ...
    public double[] getDistances() {
        double maxd = Math.min(rect.width, rect.height);
        double mind = Math.max(rect.width, rect.height);
        double avgd = (mind + maxd) / 2;
        return new double[] {2550/mind, 2550/avgd, 2550/maxd};
    }
    ...
    /**
     * Pretend we're a distance sensor.
     * We go with the max distance (ix 2) because that seems more reliable.
     * @param unit
     * @return The distance from the cube
     */
    @Override
    public double getDistance(DistanceUnit unit) {
        return isFound()?unit.fromCm(getDistances()[2]):DistanceSensor.distanceOutOfRange;
    }
